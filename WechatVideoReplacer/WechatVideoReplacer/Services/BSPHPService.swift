...
class BSPHPService {
    ...
    private static func getUserFriendlyErrorMessage(errorCode: String, serverMessage: String) -> String {
        // BSPHP 官方错误码映射 (来源: https://www.bsphp.com/chm-21.html)
        ...
        } else if lowerMessage.contains("冻结") || lowerMessage.contains("封禁") || lowerMessage.contains("banned") {
            return "卡密已被冻结，请联系客服"
        ...
        // 如果都没匹配，返回服务器原始消息（已经是中文）
        // 如果服务器消息为空或太技术化，使用通用提示
        if serverMessage.isEmpty || serverMessage.count < 5 {
